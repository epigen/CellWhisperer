...
def load_sthelar_slide(zarr_path):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            logger.warning(
                "SpatialData not available, falling back to direct zarr loading"
            )
...
def create_adata_for_slide(
    ...
    if len(patch_ids) == 0:
        raise ValueError(
            f"No patches with expression data found for sample {sample_id}"
        )
    ...
    if patches and len(patches) > 0:
        ...
        adata.uns["he_slide"] = first_patch

        # Set spot diameter for patch extraction
        adata.uns["spot_diameter_fullres"] = 224  # typical patch size
