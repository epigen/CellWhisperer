...
# redirect loggging to file snakemake.log
...
logging.info("Creating h5ad files from PathGen data...")

# Load filtered metadata
with open(snakemake.input.metadata_filtered, "r") as f:
    data = json.load(f)
logging.info(f"Loaded {len(data)} filtered entries")
...
logging.info(f"Processing {len(wsi_groups)} WSI files")
...
for file_id, entries in tqdm(wsi_groups.items(), desc="Processing WSI files"):
    try:
        ...
        if (
            snakemake.params.max_patches_per_wsi
            and len(entries) > snakemake.params.max_patches_per_wsi
        ):
        ...
        # Get WSI path from the available files using the file_id
        wsi_files_map = {Path(f).stem: Path(f) for f in snakemake.input.wsi_files}
        wsi_path = wsi_files_map.get(file_id)
        ...
        adata.uns["coordinate_system"] = (
            "center"  # Document that we use center coordinates
        )
        ...
        logging.error(f"Failed to process WSI {file_id}", exc_info=True)
        continue
...
logging.info(f"Created {len(list(output_dir.glob('*.h5ad')))} h5ad files")
logging.info(f"Generated {len(example_patches)} example patches")
