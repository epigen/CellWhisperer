...
def convert_mm_to_pixel_coordinates(
    ...
    This implements the coordinate transformation with pixel-based offsets and separate X/Y scaling:
    1. Convert mm to pixel coordinates using separate X/Y scale factors
    2. Apply pixel-based offsets
    ...
    Args:
        ...
        x_offset_pixels: X offset in pixels (already in pixel space)
        y_offset_pixels: Y offset in pixels (already in pixel space)
        scale_factor_x: Scale factor for X dimension
        scale_factor_y: Scale factor for Y dimension
        rotation_degrees: Rotation angle in degrees (default: 270.0)
    ...
    logger.info(
        f"Applying pixel offsets: x_offset={x_offset_pixels}, y_offset={y_offset_pixels}"
    )
    ...
    logger.info(
        f"Applying {rotation_degrees:.2f}° rotation around center ({center_x:.1f}, {center_y:.1f})"
    )
    ...
    x_coords_final = x_rotated + center_x + x_offset_pixels
    y_coords_final = y_rotated + center_y + y_offset_pixels
...
def _evaluate_offset_batch(args):
    ...
    for dx, dy in batch_offsets:
        ...
        for cell_x, cell_y in offset_coords:
            ...
            if (
                0 <= cell_x_int < value_channel.shape[1]
                and 0 <= cell_y_int < value_channel.shape[0]
            ):
        ...
        normalized_score = (
            score / valid_cells if valid_cells > 0 else np.inf
        )  # inf for invalid positions
        results.append(((dx, dy), normalized_score, valid_cells))

    return results


def perform_core_level_alignment(
    adata_hr, slide_wrapper, spatial_coords, dataset_name, n_jobs=None, batch_size=320
):
    """
    Perform core-level alignment using HSV value channel optimization with inner parallelization.

    For each core:
    1. Get bounding box of all cells in that core (+200px margin)
    2. Extract level-0 image patch for that core
    3. Convert to HSV and use only value channel
    4. Run 320x320 offset grid search (-320 to +320 pixels) in parallel
    ...
    Args:
        ...
        batch_size: Number of offset positions to evaluate per batch (default: 320)

    Returns:
        tuple: (aligned_coords, cell_intensities)
            - aligned_coords: numpy array of aligned spatial coordinates
            - cell_intensities: numpy array of value channel intensities at aligned positions
    """
    ...
    if "core_id" not in adata_hr.obs.columns:
        ...
        return spatial_coords, np.full(len(spatial_coords), np.nan)

    # Copy coordinates for modification
    aligned_coords = spatial_coords.copy()
    # Initialize intensity array with NaN
    all_cell_intensities = np.full(len(spatial_coords), np.nan)
    ...
    if n_jobs is None:
        n_jobs = cpu_count()
    logger.info(
        f"Using inner parallelization with {n_jobs} workers, batch size {batch_size}"
    )
    ...
    for core in cores:
        ...
        # Get bounding box with 200px margin
        ...
        logger.info(
            f"Core {core} bounding box: ({bbox_x_min}, {bbox_y_min}) to ({bbox_x_max}, {bbox_y_max}), size: {bbox_width}x{bbox_height}"
        )

        # Extract level-0 image patch
        try:
            region = slide_wrapper.read_region(
            ...
            logger.info(
                f"Extracted {value_channel.shape} value channel for core {core}"
            )
        ...
        offset_range = range(-320, 320)
        all_offsets = [(dx, dy) for dx in offset_range for dy in offset_range]

        logger.info(
            f"Starting grid search for core {core}: {len(offset_range)}x{len(offset_range)} = {len(all_offsets)} positions"
        )

        # Create batches of offsets for parallel processing
        batches = [
            all_offsets[i : i + batch_size]
            for i in range(0, len(all_offsets), batch_size)
        ]
        ...
        with ProcessPoolExecutor(max_workers=n_jobs) as executor:
            with tqdm(
                total=len(all_offsets), desc=f"Core {core} alignment", unit="pos"
            ) as pbar:
                futures = {
                    executor.submit(_evaluate_offset_batch, task): len(task[2])
                    for task in tasks
                }
                    ...
                    for (dx, dy), score, valid_cells in batch_results:
                        if valid_cells > 0 and score < best_score:
                            ...
                            pbar.set_description(
                                f"Core {core} alignment (best: {dx:+3d},{dy:+3d} = {best_score:.1f})"
                            )
        ...
        if all_results:
            valid_results = [
                (offset, score, cells)
                for offset, score, cells in all_results
                if cells > 0
            ]
            ...
            if valid_results:
                ...
                logger.info(
                    f"Core {core}: Best offset = ({dx_best}, {dy_best}), score = {best_score:.2f}"
                )

                # Apply best offset to all cells in this core
                aligned_coords[core_mask] = spatial_coords[core_mask] + np.array(
                    [dx_best, dy_best]
                )

                # Extract pixel intensities at aligned cell positions for distribution plot
                aligned_rel = core_coords_relative + np.array([dx_best, dy_best])
                cell_intensities = []
                core_indices = np.where(core_mask)[0]
                for idx, (cell_x, cell_y) in enumerate(aligned_rel):
                    cell_x_int, cell_y_int = int(round(cell_x)), int(round(cell_y))
                    if (
                        0 <= cell_x_int < value_channel.shape[1]
                        and 0 <= cell_y_int < value_channel.shape[0]
                    ):
                        intensity = value_channel[cell_y_int, cell_x_int]
                        cell_intensities.append(intensity)
                        all_cell_intensities[core_indices[idx]] = intensity

                logger.info(
                    f"Core {core}: Extracted intensities for {len(cell_intensities)}/{len(core_coords)} cells "
                    f"(mean={np.mean(cell_intensities):.1f}, median={np.median(cell_intensities):.1f})"
                )

                # Create visualizations
                try:
                    # Get project directory once for both plots
                    project_dir = Path(
                    ...
                    results_dir = project_dir / f"results/{dataset_name}"
                    results_dir.mkdir(parents=True, exist_ok=True)

                    # Plot 1: Alignment visualization
                    ...
                    plot_path = results_dir / f"core_{core}_alignment.png"
                    plt.tight_layout()
                    ...
                    # Plot 2: Cell intensity distribution
                    if cell_intensities:
                        ...
                        density = stats.gaussian_kde(cell_intensities)
                        x_range = np.linspace(
                            min(cell_intensities), max(cell_intensities), 320
                        )
                        plt.plot(
                            x_range, density(x_range), "r-", linewidth=2, label="KDE"
                        )

                        plt.title(
                            f"Core {core} Cell Intensity Distribution\n"
                            f"Mean: {np.mean(cell_intensities):.1f}, "
                            f"Median: {np.median(cell_intensities):.1f}, "
                            f"N={len(cell_intensities)} cells"
                        )
                        plt.xlabel("Value Channel Intensity (0-255)")
                        plt.ylabel("Density")
                        plt.legend()
                        plt.grid(True, alpha=0.3)

                        intensity_plot_path = (
                            results_dir / f"core_{core}_intensity_distribution.png"
                        )
                        plt.tight_layout()
                        plt.savefig(intensity_plot_path, dpi=150, bbox_inches="tight")
                        plt.close()

                        logger.info(
                            f"Saved intensity distribution to {intensity_plot_path}"
                        )
    ...
    return aligned_coords, all_cell_intensities
