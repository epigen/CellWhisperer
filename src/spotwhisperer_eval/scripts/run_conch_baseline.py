#!/usr/bin/env python3
# Minimal wrapper to run CONCH baseline exactly like external script
import sys
import os
import numpy as np
import pandas as pd
...
for tiff_path, h5ad_path in zip(tiff_files, h5ad_files):
    ...
    for i in range(len(adata)):
        ...
        inputs = preprocess(image).unsqueeze(0).to(device=device)
        ...
        row_terms1 = {**row_base, **{labels_1[j]: sim_scores_1[j] for j in range(len(sim_scores_1))}}
        row_terms2 = {**row_base, **{labels_2[j]: sim_scores_2[j] for j in range(len(sim_scores_2))}}
