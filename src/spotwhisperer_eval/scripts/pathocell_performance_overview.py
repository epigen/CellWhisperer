...
results_a = [Path(p) for p in snakemake.input.results_a]
results_b = [Path(p) for p in snakemake.input.results_b]
...
def load_per_class(files):
    ...
    for fp in files:
        ...
        if metric_col not in df.columns:
            raise KeyError(f"Requested metric '{metric_col}' not found in per-class file: {fp}")
...
def load_results_json(files):
    ...
    for fp in files:
        ...
        # parse dataset from filename
        stem = fp.stem
        # Support both older _results_seed and current _prediction_seed patterns
        suffix_pats = [
        ...
        if metric_col not in obj:
            raise KeyError(f"Requested metric '{metric_col}' not found in results JSON: {fp}")
        rows.append({"dataset": dataset, metric_col: obj[metric_col]})
...
if use_json:
    if prediction_level != "patch":
        raise ValueError(f"Metric '{metric_col}' is only available for prediction_level='patch'")
    if scatter_unit == "class":
        raise ValueError(f"Metric '{metric_col}' is not class-level; use scatter_unit='dataset'")
    ...
    exclude_labels = {"Other cells", "Background", "Background cells", "A sample of Background cells"}
...
if scatter_unit == "dataset":
    ...
    if use_json:
        raise ValueError(f"Metric '{metric_col}' is not class-level; set scatter_unit='dataset'")
    a_unit = (
...
if snakemake.params.plot_type == "violin":
    if scatter_unit == "dataset":
        if use_json:
            ...
            a_vals = (
                a_seed_mean.groupby("dataset", as_index=False)[metric_col].mean()[metric_col].to_numpy()
            )
            b_vals = (
                b_seed_mean.groupby("dataset", as_index=False)[metric_col].mean()[metric_col].to_numpy()
            )
    else:
        a_vals = (
            a_seed_mean.groupby("class_label", as_index=False)[metric_col].mean()[metric_col].to_numpy()
        )
        b_vals = (
            b_seed_mean.groupby("class_label", as_index=False)[metric_col].mean()[metric_col].to_numpy()
        )
    long_df = pd.DataFrame({"model": np.repeat([model_a, model_b], [len(a_vals), len(b_vals)]), metric_col: np.concatenate([a_vals, b_vals])})
    ax = sns.violinplot(data=long_df, y="model", x=metric_col, inner="box", palette=["#2b7b9c", "#bfbfbf"], orient="h")
    ...
    if np.isfinite(lims_min) and np.isfinite(lims_max):
        ...
        ax.plot([lims_min - pad, lims_max + pad], [lims_min - pad, lims_max + pad], linestyle="--", color="#888888", linewidth=1)
...
ax.set_title(f"{metric_col}: {model_b} vs {model_a} | {prediction_level} | by {mode_label} ({snakemake.params.plot_type})")
handles, labels = ax.get_legend_handles_labels()
labels = ["p < 0.05" if l == "True" else ("p \u2265 0.05" if l == "False" else l) for l in labels]
if labels:
