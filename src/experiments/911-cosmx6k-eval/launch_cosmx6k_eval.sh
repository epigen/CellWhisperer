...
# CosMx6K Evaluation Experiment (911) - SLURM Launch Script
...
EXPERIMENT_DIR="$PROJECT_ROOT/src/experiments/911-cosmx6k-eval"
...
declare -a DELTA_CONFIGS=(
    ...
    "uul_locking_config.yaml"
    "good_quality_tmas_config.yaml"
    "lower_lr.yaml"
)

echo "Launching CosMx6K evaluation jobs..."
...
submit_job "cosmx6k-baseline" "--config base_config.yaml" "Baseline run with base config only"

# Launch jobs for each delta config
for config in "${DELTA_CONFIGS[@]}"; do
    job_name="cosmx6k-${config%.yaml}"
...
echo "  - cosmx6k-baseline (baseline with MLP + ramped CNN + uLL)"
for config in "${DELTA_CONFIGS[@]}"; do
    case "$config" in
        "disable_cell_level_config.yaml") echo "  - cosmx6k-no-cell-level" ;;
        "cell_cnn_only_config.yaml") echo "  - cosmx6k-cell-cnn-only" ;;
        "finetune_geneformer_config.yaml") echo "  - cosmx6k-finetune-uul" ;;
        "uul_locking_config.yaml") echo "  - cosmx6k-lul" ;;
        "good_quality_tmas_config.yaml") echo "  - cosmx6k-good-quality-tmas" ;;
        "lower_lr.yaml") echo "  - cosmx6k-lower-lr" ;;
    esac
done
