...
def plot_term_score_umaps(adata, term, scores_this_term, df_stack, outdir):
    ...
    top_5_cluster_labels=df_stack[df_stack["term_without_prefix"]==term]["cluster_label"].values[:5]
    label_score_dict={x:round(float(y),1) for x, y in zip(top_5_cluster_labels,df_stack[df_stack["term_without_prefix"]==term]["logits"].values[:5])}
    
    adata.obs[f"per_transcriptome_score_for_{term}"] = scores_this_term
    adata.obs["cluster_label_top_5"]=[f"{x}\nScore_cluster:{round(label_score_dict[x],1)}" if x in top_5_cluster_labels else "other" for x in adata.obs["cluster_label"]]
    order=[f"{x}\nScore_cluster:{round(label_score_dict[x],1)}" for x in top_5_cluster_labels]

    order.append("other")

    adata.obs["cluster_label_top_5"]=pd.Categorical(adata.obs["cluster_label_top_5"], categories=order, ordered=True)

    langauge_annots_top5=adata.obs.sort_values(by=f"per_transcriptome_score_for_{term}",ascending=False).head(5)["natural_language_annotation"].tolist()


    # plot
    best_score= adata.obs[f"per_transcriptome_score_for_{term}"].max()
    fig, axes = plt.subplots(2, 2, figsize=(15, 7), gridspec_kw={"height_ratios": [1, 0.3]})

    # Score per transcriptome
    sc.pl.embedding(adata,
                    basis="X_umap", color=f"per_transcriptome_score_for_{term}",
                    cmap="RdBu_r", 
                    vmin=-best_score,
                    vmax=best_score, show=False, ax=axes[0][0])

    # Cluster labels
    sc.pl.embedding(adata[adata.obs["cluster_label_top_5"]=="other"],
                    basis="X_umap", color=["cluster_label_top_5"], show=False,
                    ax=axes[0][1], palette={"other":"silver"},
                    s=0.5)
    axes[0][1].get_legend().remove()
    sc.pl.embedding(adata[adata.obs["cluster_label_top_5"]!="other"],
                    basis="X_umap", color=["cluster_label_top_5"], show=False, ax=axes[0][1],
                    legend_loc="on data", legend_fontsize=6,
                    s=0.5)

    plt.sca(axes[1][0])
    plt.text(0.5, 0.5, "Examples from top cluster: \n"+("\n".join(langauge_annots_top5)), horizontalalignment='left', verticalalignment='center', transform=axes[1][0].transAxes, fontsize=9)
...
(pl_model_cellwhisperer,text_processor_cellwhisperer,transcriptome_processor_cellwhisperer) = load_cellwhisperer_model(model_path=repaired_path, eval=True)

# Prepare annData
adata=adata[~np.isnan(npz['transcriptome_embeds']).any(axis=1)] # 98 transcriptomes have nan values
embeds_not_nan=npz['transcriptome_embeds'][~np.isnan(npz['transcriptome_embeds']).any(axis=1)] # subset also the npz file for score calculation later
adata.obs["cluster_label"]=adata.obs["leiden"].apply(lambda x: f"{x} ({language_labels.iloc[int(x)]})")
adata.obs["cluster_label"] = [x[:60].replace("\n", "") for x in adata.obs["cluster_label"]]
annotations_per_transcriptome_keys_list=list(annotations_per_transcriptome.keys())
adata.obs["natural_language_annotation"]=[annotations_per_transcriptome[annotations_per_transcriptome_keys_list[int(idx)]] for idx in adata.obs.index]
...
scores, annot = score_transcriptomes_vs_texts(
    ...
    transcriptome_input=torch.tensor(embeds_not_nan, 
...
for term, df_this_term in df_stack.groupby("term_without_prefix"):
    ...
    score_diff=df_this_term["logits"].iloc[0]-df_this_term["logits"].iloc[compare_top_vs_clusternumber]
    term_dict[term]={"term_without_prefix":term, "score_diff":score_diff, "top_cluster":df_this_term["cluster_label"].iloc[0], "top_score":df_this_term["logits"].iloc[0]}
df_score_diff=pd.DataFrame.from_dict(term_dict,orient="index").sort_values(by="score_diff",ascending=False)

# keep at most n terms per cluster but keep order 
df_score_diff=df_score_diff.groupby("top_cluster").head(max_terms_per_cluster).sort_values(by="score_diff",ascending=False)
top_terms=df_score_diff["term_without_prefix"].tolist()[:max_terms_total]

top_terms+=df_stack.drop_duplicates(subset="term_without_prefix")["term_without_prefix"][:20].tolist() # add the overall top 20 terms
...
scores_zscored_over_terms=(scores_all_terms[:,1:]-scores_all_terms[:,1:].mean(axis=0))/scores_all_terms[:,1:].std(axis=0)
...
scores_top_terms = torch.cat([normed_score_sum_of_all_terms.unsqueeze(1), scores_top_terms], dim=1)
...
sc.pl.umap(adata, color=["cluster_label"], 
           ax=ax, palette=list(matplotlib.colors.CSS4_COLORS.values()),
           show=False,
           legend_fontsize=6,
           s=0.75)
...
for i,term in enumerate(top_terms):
    plot_term_score_umaps(adata=adata,
                          term=term,
